...
// Diamond Packages Data - Fixed structure as requested
const DIAMOND_PACKAGES = {
  double_bonus: [
    { diamonds: "50+50", price: 80 },
    { diamonds: "150+150", price: 260 },
    { diamonds: "250+250", price: 370 },
    { diamonds: "500+500", price: 680 }
  ],
  weekly_pass: { price: 130, maxQty: 10 },
  small_packs: [
...
const DEFAULT_USERS = [
  ...
  { username: "vip_user", password: "vip789", role: "customer", lastModified: Date.now() }
];

// Google Drive Sync Class - Only for admin
class DriveSync {
  constructor() {
...
class PKStoreApp {
  ...
  async initialize() {
    ...
    this.loadLocalUsers();
    console.log('Users loaded:', this.users.length, this.users);
  ...
  loadLocalUsers() {
    try {
      ...
      if (stored) {
        ...
        this.saveLocalUsers();
        console.log('Using default users:', this.users);
      }
    } catch (error) {
  ...
  setupEventListeners() {
    ...
    setTimeout(() => {
      ...
      // Google Drive connection - only for admin
      ...
      // Weekly pass quantity controls
      const weeklyDecreaseBtn = document.getElementById('weekly-decrease');
      const weeklyIncreaseBtn = document.getElementById('weekly-increase');
      const weeklyOrderBtn = document.getElementById('weekly-order-btn');
      
      if (weeklyDecreaseBtn) {
        weeklyDecreaseBtn.addEventListener('click', () => this.changeWeeklyQuantity(-1));
      }
      if (weeklyIncreaseBtn) {
        weeklyIncreaseBtn.addEventListener('click', () => this.changeWeeklyQuantity(1));
      }
      if (weeklyOrderBtn) {
        weeklyOrderBtn.addEventListener('click', () => this.orderWeeklyPass());
      }
  ...
  handleLogin(e) {
    ...
    if (user) {
      ...
      // Initialize Google Drive for admin users only
      if (user.role === 'admin') {
  ...
  async initializeAdminFeatures() {
    ...
    adminElements.forEach(element => {
      ...
      if (element.style.display === 'flex' || element.classList.contains('sync-indicator')) {
        element.classList.add('flex');
      }
    });
    
    // Initialize Google Drive API for admin
    try {
      await this.driveSync.initialize();
      console.log('Drive sync initialized for admin');
    } catch (error) {
  ...
  handleRegister(e) {
    ...
    // Auto-sync if admin is connected
    if (this.currentUser && this.currentUser.role === 'admin' && this.driveSync.isAuthenticated) {
  ...
  renderDiamondPackages() {
    ...
    // Render double bonus packages
    const doubleBonusContainer = document.getElementById('double-bonus');
    if (doubleBonusContainer) {
      doubleBonusContainer.innerHTML = '';
      DIAMOND_PACKAGES.double_bonus.forEach(pack => {
        doubleBonusContainer.appendChild(this.createPackageCard(pack, 'special'));
      });
    }
  ...
  createPackageCard(pack, type = 'normal') {
    const card = document.createElement('div');
    card.className = type === 'special' ? 'package-card special' : 'package-card';
    
    const diamondDisplay = typeof pack.diamonds === 'string' ? pack.diamonds : `${pack.diamonds}`;
    
    card.innerHTML = `
      <div class="package-header">
        <div class="diamond-count">${diamondDisplay} 💎</div>
        <div class="package-price">₹ ${pack.price.toLocaleString()}</div>
      </div>
      ${type === 'special' ? '<div class="package-bonus"><span class="bonus-text">First Purchase Bonus!</span></div>' : ''}
    ...
    button.addEventListener('click', () => {
      this.showOrderModal(`${diamondDisplay} 💎`, `₹ ${pack.price.toLocaleString()}`);
    });
    
    return card;
  }

  changeWeeklyQuantity(change) {
    const newQuantity = this.weeklyPassQuantity + change;
    if (newQuantity >= 1 && newQuantity <= DIAMOND_PACKAGES.weekly_pass.maxQty) {
      this.weeklyPassQuantity = newQuantity;
      const quantityDisplay = document.getElementById('weekly-quantity');
      if (quantityDisplay) {
        quantityDisplay.textContent = this.weeklyPassQuantity;
      }
    }
  }

  orderWeeklyPass() {
    const totalPrice = this.weeklyPassQuantity * DIAMOND_PACKAGES.weekly_pass.price;
    const packageName = `${this.weeklyPassQuantity}x Weekly Pass`;
    this.showOrderModal(packageName, `₹ ${totalPrice.toLocaleString()}`);
  }
...
// Auto-sync functionality - only for admin users
setInterval(async () => {
  if (app && app.currentUser && app.currentUser.role === 'admin' && app.driveSync.isAuthenticated && app.isLoggedIn) {
    // Perform background sync every 5 minutes for admin only
    console.log('Performing background sync...');
    await app.syncUserData();
  }
}, 300000); // 5 minutes

// Handle page visibility change for sync - only for admin
document.addEventListener('visibilitychange', async () => {
  if (!document.hidden && app && app.currentUser && app.currentUser.role === 'admin' && app.driveSync.isAuthenticated && app.isLoggedIn) {
    // Sync when page becomes visible for admin only
