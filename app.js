...
const DEFAULT_USERS = [
  ...
  { username: "customer2", password: "pass456", role: "customer" },
  { username: "vip_user", password: "vip789", role: "vip" }
];
...
function initializeApp() {
  ...
  if (!localStorage.getItem('authorizedUsers')) {
    localStorage.setItem('authorizedUsers', JSON.stringify(DEFAULT_USERS));
  }
  ...
  authorizedUsers.forEach((user, index) => {
    ...
    userItem.innerHTML = `
      ...
      <button class="remove-user-btn" onclick="removeUser(${index})" 
              ${user.username === 'admin' ? 'disabled title="Cannot remove admin user"' : ''}>
        Remove
      </button>
    `;
...
function handleAddUser(e) {
  ...
  // Add new user
  authorizedUsers.push({ username, password, role });
...
function verifyBeforePayment(product, price) {
  ...
  document.getElementById('verify-price').textContent = `₹ ${price}`;
